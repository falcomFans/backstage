						...
						<div class="form-three widget-shadow">
							<form class="form-horizontal" action="" method="post">
								...
								<div class="form-group">
									<label for="focusedinput" class="col-sm-2 control-label">组名</label>
									<div class="col-sm-8">
										<input type="text" class="form-control1" id="focusedinput" name="role_name" value="<?php echo ($info['role_name']); ?>">
									</div>
								</div>
								<?php if(($info['role_id']) != "1"): ?><div class="form-group">
										<label for="checkbox" class="col-sm-2 control-label">权限分配</label>
									</div>
									<?php if(is_array($authAA)): foreach($authAA as $key=>$v): ?><div class="form-group">
											<label for="checkbox" class="col-sm-2 control-label"><input type="checkbox" name="auth_id[]"  value="<?php echo ($v["auth_id"]); ?>" 
											<?php if(in_array(($v[auth_id]), is_array($info["role_auth_ids"])?$info["role_auth_ids"]:explode(',',$info["role_auth_ids"]))): ?>checked<?php endif; ?> ><?php echo ($v["auth_name"]); ?></label></label>
											<div class="col-sm-8">
												<?php if(is_array($authBB)): foreach($authBB as $key=>$vv): if(($vv['auth_pid']) == $v['auth_id']): ?><div class="checkbox-inline"><label><input type="checkbox" name="auth_id[]"  value="<?php echo ($vv["auth_id"]); ?>" 
														<?php if(in_array(($vv[auth_id]), is_array($info["role_auth_ids"])?$info["role_auth_ids"]:explode(',',$info["role_auth_ids"]))): ?>checked<?php endif; ?> > <?php echo ($vv["auth_name"]); ?></label></div><?php endif; endforeach; endif; ?>
											</div>
...
<script src="<?php echo C('LAY_URL');?>layer.js"}></script>
	...
	<script src="<?php echo C('JS_URL');?>bootstrap.js">
