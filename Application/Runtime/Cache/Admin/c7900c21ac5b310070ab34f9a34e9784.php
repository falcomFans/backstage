                        ...
                        <form class="form-horizontal" action="" method="post" onsubmit="return isTrue()">
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <tr>
                                        <th scope="row">账号</th>
                                        <td><input type="text" class="form-control1" id="focusedinput" name="mg_name" value="<?php echo ($info["mg_name"]); ?>">                                            </td>
                                        <td id="focusedinput_td">用于登录的用户名</td>
                                    </tr>
                                    <tr>
                                        <th scope="row">密码</th>
                                        <td><input type="password" class="form-control1" disabled id="inputPassword1" name="mg_pwd" placeholder="Password">                                            </td>
                                        <td id="inputPassword_td1">暂时不开放平台修改密码功能</td>
                                    </tr>
                                    <tr>
                                        <th scope="row">确认密码</th>
                                        <td><input type="password" class="form-control1" disabled id="inputPassword2" placeholder="Password"></td>
                                        <td id="inputPassword_td2">暂时不开放平台修改密码功能</td>
                                    </tr>
                                    <?php if(($info['role_id']) != "1"): ?><tr>
                                            <th scope="row">管理组名称</th>
                                            <td><select name="role_name" id="selector" class="form-control1">
                                    <?php if(is_array($role_info)): foreach($role_info as $key=>$v): ?><!--<?php if(($v['role_id']) != "1"): ?>-->
										    <option value="<?php echo ($v['role_id']); ?>" <?php if(($v['role_id']) == $info['role_id']): ?>selected<?php endif; ?>><?php echo ($v["role_name"]); ?></option>
                                        <!--<?php endif; ?>--><?php endforeach; endif; ?>
									</select></td>
                                            <td>管理组名称，不同管理组权限不同，具体可参见管理组列表或询问超级管理员</td>
                                        </tr>
                                        <tr>
                                            <th scope="row">拥有的权限代号</th>
                                            <td id="disabledinput1" ><input disabled="" type="text" class="form-control1"  value="<?php echo ($info["role_auth_ids"]); ?>"></td>
                                            <td>该账号拥有的权限，具体可参见权限列表或询问超级管理员</td>
                                        </tr>
                                        <tr>
                                            <th scope="row">控制器-方法</th>
                                            <td id="disabledinput2"><input disabled="" type="text" class="form-control1"  value="<?php echo ($info["role_auth_ac"]); ?>"></td>
                                            <td>该账号可以访问的路由，具体可参见权限列表或询问超级管理员</td>
                                        </tr><?php endif; ?>
                                </tbody>
                            </table>
...
<script>
    ...
    $("#selector").change(function (e) { 
        ...
        $.ajax({
            type: "post",
            url: "<?php echo U('Role/getOne');?>",
            data: "role_id=" + $("#selector").val(),
            dataType: "json",
            success: function (msg) {
                if (msg["status"]==200) 
                {
                    $("#disabledinput1").html("<input disabled='' type='text' class='form-control1'  value="+msg["role_auth_ids"]+">");
                    $("#disabledinput2").html("<input disabled='' type='text' class='form-control1'  value="+msg["role_auth_ac"]+">");
                }
